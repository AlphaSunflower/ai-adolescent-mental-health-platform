...
export function FieldManager() {
  ...
  const [form, setForm] = useState<Record<string,unknown>>({ name: "", code: "", icon: "", description: "", sortOrder: "", status: 1 });

  const fetchData = useCallback(async () => {
    try {
      const res = await httpClient.get<Record<string,unknown>[]>("/admin/psychologist/consultation-fields");
  ...
  const openAdd = () => { setEditingItem(null); setForm({ name: "", code: "", icon: "", description: "", sortOrder: "", status: 1 }); setDialogVisible(true); };
  const openEdit = (item: Record<string,unknown>) => { setEditingItem(item); setForm({ name: item.name || "", code: item.code || "", icon: item.icon || "", description: item.description || "", sortOrder: item.sortOrder || "", status: (item.status as number) ?? 1 }); setDialogVisible(true); };
  ...
  const handleToggleStatus = async (row: Record<string,unknown>) => {
    ...
    try {
      await httpClient.put(`/admin/psychologist/consultation-field/${row.id}`, { ...row, status: newStatus });
      fetchData();
    } catch { /* ignore */ }
  };
  ...
  const handleSave = async () => {
    try {
      if (editingItem) {
        await httpClient.put(`/admin/psychologist/consultation-field/${editingItem.id}`, form);
      } else {
        await httpClient.post("/admin/psychologist/consultation-field", form);
      }
  ...
  const handleDelete = async (id: unknown) => {
    ...
    try { await httpClient.delete(`/admin/psychologist/consultation-field/${id}`); fetchData(); } catch { /* ignore */ }
  };
        ...
        <table style={{ width:"100%", borderCollapse:"collapse", border:`1px solid ${s.border}` }}>
          <thead>
            <tr style={{ backgroundColor:s.bg }}>
              {["ID","领域名称","编码","图标","描述","排序","状态","操作"].map(h => <th key={h} style={{ padding:"12px 8px", textAlign:"left", fontSize:"13px", color:s.text3, fontWeight:600, borderBottom:`1px solid ${s.border}` }}>{h}</th>)}
            </tr>
          </thead>
          <tbody>
            {data.records.map((row, i) => (
              <tr key={i} style={{ backgroundColor: i%2===0?"#fff":s.bg }}>
                ...
                <td style={{ padding:"12px 8px", borderBottom:`1px solid ${s.border}`, fontSize:"13px" }}>{row.name as string}</td>
                <td style={{ padding:"12px 8px", borderBottom:`1px solid ${s.border}`, fontSize:"13px" }}>{row.code as string}</td>
                <td style={{ padding:"12px 8px", borderBottom:`1px solid ${s.border}`, fontSize:"13px" }}>{row.icon as string}</td>
                <td style={{ padding:"12px 8px", borderBottom:`1px solid ${s.border}`, fontSize:"13px", maxWidth:"150px", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{row.description as string}</td>
      ...
      {detailVisible && currentItem && (
        <div style={{ position:"fixed", inset:0, backgroundColor:"rgba(0,0,0,0.5)", display:"flex", justifyContent:"center", alignItems:"center", zIndex:1000 }} onClick={() => setDetailVisible(false)}>
          <div style={{ backgroundColor:s.white, borderRadius:"8px", padding:"24px", width:"480px" }} onClick={(e) => e.stopPropagation()}>
            <h3 style={{ margin:"0 0 20px", fontSize:"18px", color:s.text }}>领域详情</h3>
            ...
            <div style={{ marginBottom:"12px" }}><span style={{ fontSize:"13px", color:s.text3 }}>图标：</span><span style={{ fontSize:"13px" }}>{currentItem.icon as string}</span></div>
            <div style={{ marginBottom:"12px" }}><span style={{ fontSize:"13px", color:s.text3 }}>描述：</span><span style={{ fontSize:"13px" }}>{currentItem.description as string || "-"}</span></div>
      ...
      {dialogVisible && (
        <div style={{ position:"fixed", inset:0, backgroundColor:"rgba(0,0,0,0.5)", display:"flex", justifyContent:"center", alignItems:"center", zIndex:1000 }} onClick={() => setDialogVisible(false)}>
          <div style={{ backgroundColor:s.white, borderRadius:"8px", padding:"24px", width:"480px" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ marginBottom:"16px" }}><label style={{ display:"block", marginBottom:"6px", fontSize:"13px", color:s.text2 }}>图标</label><input value={form.icon as string} onChange={(e) => setForm({...form, icon:e.target.value})} style={{ width:"100%", height:"36px", padding:"0 12px", border:`1px solid ${s.border}`, borderRadius:s.radius, boxSizing:"border-box" }} /></div>
            <div style={{ marginBottom:"16px" }}><label style={{ display:"block", marginBottom:"6px", fontSize:"13px", color:s.text2 }}>描述</label><textarea value={form.description as string} onChange={(e) => setForm({...form, description:e.target.value})} rows={2} style={{ width:"100%", padding:"8px 12px", border:`1px solid ${s.border}`, borderRadius:s.radius, boxSizing:"border-box", resize:"vertical" }} /></div>
