...
export function QualificationManager() {
  ...
  const [form, setForm] = useState<Record<string,unknown>>({ name: "", code: "", description: "", sortOrder: "", status: 1 });
  ...
  const openAdd = () => { setEditingItem(null); setForm({ name: "", code: "", description: "", sortOrder: "", status: 1 }); setDialogVisible(true); };
  const openEdit = (item: Record<string,unknown>) => { setEditingItem(item); setForm({ name: item.name || "", code: item.code || "", description: item.description || "", sortOrder: item.sortOrder || "", status: (item.status as number) ?? 1 }); setDialogVisible(true); };
  ...
  const handleToggleStatus = async (row: Record<string,unknown>) => {
    ...
    try {
      await httpClient.put(`/admin/psychologist/qualification/${row.id}`, { ...row, status: newStatus });
      fetchData();
    } catch { /* ignore */ }
  };
    ...
    <div style={{ padding:"20px", backgroundColor:s.bg, minHeight:"100%" }}>
      <div style={{ backgroundColor:s.white, borderRadius:"8px", boxShadow:s.shadow, padding:"20px" }}>
        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"20px" }}>
          <h3 style={{ margin:0, fontSize:"18px", color:s.text }}>资质管理</h3>
          <button onClick={openAdd} style={{ height:"36px", padding:"0 20px", backgroundColor:s.primary, color:"#fff", border:"none", borderRadius:s.radius, cursor:"pointer" }}>新增资质</button>
        </div>
        <table style={{ width:"100%", borderCollapse:"collapse", border:`1px solid ${s.border}` }}>
          <thead>
            <tr style={{ backgroundColor:s.bg }}>
              {["ID","资质名称","编码","描述","排序","状态","操作"].map(h => <th key={h} style={{ padding:"12px 8px", textAlign:"left", fontSize:"13px", color:s.text3, fontWeight:600, borderBottom:`1px solid ${s.border}` }}>{h}</th>)}
            </tr>
          </thead>
          <tbody>
      ...
      {detailVisible && currentItem && (
        <div style={{ position:"fixed", inset:0, backgroundColor:"rgba(0,0,0,0.5)", display:"flex", justifyContent:"center", alignItems:"center", zIndex:1000 }} onClick={() => setDetailVisible(false)}>
          <div style={{ backgroundColor:s.white, borderRadius:"8px", padding:"24px", width:"480px" }} onClick={(e) => e.stopPropagation()}>
            <h3 style={{ margin:"0 0 20px", fontSize:"18px", color:s.text }}>资质详情</h3>
